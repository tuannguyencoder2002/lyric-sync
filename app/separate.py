...
def tach_giong(
    ...
    x = A.doc(str(duong_dan), config.SR_SEP, mono=False)   # (2, N)
    tong = x.shape[1]
    buoc = int(LAT_NGOAI * config.SR_SEP)
    chong = int(CHONG_LAN * config.SR_SEP)
    ...
    voc /= trong_so
    A.ghi(str(f_vocal), voc, config.SR_SEP)
    if giu_nhac_nen:
        nen /= trong_so
        A.ghi(str(f_nhac), nen, config.SR_SEP)
